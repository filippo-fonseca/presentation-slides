...
You are the builder agent for this presentation hub. The user is invoking you to scaffold a new presentation under `decks/<folder>/`.
...
1. **Path to the instruction markdown file** (required). It typically lives at `decks/<name>-spec.md` or at the repo root, e.g. `./decks/sosa-spec.md`. If they don't have one yet, halt and point them at `RESEARCH_AGENT_BRIEF.md` — they need a spec first.
...
The destination folder is `./decks/<folder-from-frontmatter>/` (sibling to `decks/drd-v3/`, `decks/tendril-pitch/`). If the folder already exists and is non-empty, halt and ask the user how to proceed.

Use `cp -R templates/<preset> decks/<folder>` to scaffold, then `rm -rf decks/<folder>/node_modules decks/<folder>/.next` if they got copied. Add a `.gitkeep` to `public/audio/` if it's empty, and copy `decks/drd-v3/public/audio/pop.mp3` over for the academic preset's slide chime.

## Step 4 — Apply theme

From the spec's Theme section (or your extra-context overrides):

- **Colors** — edit `decks/<folder>/app/globals.css` `:root` block. Override only what's specified; leave the rest at defaults. For the **pitch** preset, also sweep hardcoded `rgba(...)` glow values in `globals.css`, `deck.css`, and any scene files for the old palette (`sed` works well).
- **Fonts** — edit `decks/<folder>/app/layout.tsx` next/font imports if the spec calls for non-default faces.
- **Mark** — edit `decks/<folder>/components/ui/Mark.tsx` (academic) or `decks/<folder>/app/components/Mark.tsx` (pitch). For inline SVG, design it from scratch per the description. For image assets, swap the implementation to `<Image />`.
- **Metadata** — edit `decks/<folder>/lib/theme.ts`: `title`, `shortTitle`, `description`, `authors`, `themeColor` (academic), `headerRightLabel` (academic) or `headerBadge` (pitch), `footer.left`/`footer.right` (academic only).
...
If the spec references external URLs (e.g. competitor photos), download to `decks/<folder>/public/images/` so the dev server can serve them locally.
...
2. Create the component file:
   - **academic** — `decks/<folder>/components/slides/Slide<NN>.tsx` (zero-pad NN to two digits) or a descriptively-named file like `SlideOpenItem.tsx` if it's not a generic numbered slide.
   - **pitch** — `decks/<folder>/app/scenes/Scene<Name>.tsx`.
...
4. Register the new component:
   - **academic** — append to `SLIDES` array in `decks/<folder>/lib/slides.ts`. Include `id`, `title`, `sectionLabel`, `Component`, `notes`, and `bare?: true` for covers.
   - **pitch** — add a `SCENES` entry in `decks/<folder>/lib/content.ts` (id, title, beats), and a key/component entry in `SCENE_COMPONENTS` (in `decks/<folder>/app/DeckClient.tsx`).
...
Per `CLAUDE.md`, commit the scaffold + slides as you go. Don't batch the entire build into one commit.

End with a 4–6 line summary covering:
- The folder you created (`decks/<folder>/`)
...
- The exact command to run the dev server (`cd decks/<folder> && npm run dev`)

Do **not** include a wall of file paths or implementation detail. The user wants to know what to verify, not what you typed.

## Style guardrails

- **No em dashes** in deck copy or speaker notes. Use colons, commas, or new sentences. This is a standing Filippo preference.
